defmodule TheArkWeb.TeacherLive do
  ...
  def handle_event("assign_subjects_to_teacher", %{"teacher_id" => teacher_id}, %{assigns: %{subject_options: subject_options, choosen_class_id: choosen_class_id}} = socket) do
  ...
  @impl true
  def handle_event("subject_teacher_change", %{"prev_teacher_id" => prev_id, "subject_teacher_change" => %{"teacher_id" => new_id}}, socket) do
      ...
      <div>
        ...
        <%= for teacher <- @teachers do %>
          <div class="grid grid-cols-6 items-center pb-2">
            ...
            <div>
              <.button class={"#{((@edit_teacher_id == teacher.id) or (@delete_teacher_id == teacher.id) or (@assign_subjects_to_teacher_id == teacher.id)) && "hidden"}"} phx-click="edit_teacher_id" phx-value-teacher_id={teacher.id}>Edit</.button>
              <.button class={"#{((@edit_teacher_id == teacher.id) or (@delete_teacher_id == teacher.id) or (@delete_teacher_id == teacher.id) or (@assign_subjects_to_teacher_id == teacher.id)) && "hidden"}"} phx-click="delete_teacher_id" phx-value-teacher_id={teacher.id}>Delete</.button>
              <.button class={"mt-1 #{((@edit_teacher_id == teacher.id) or (@delete_teacher_id == teacher.id) or (@delete_teacher_id == teacher.id) or (@assign_subjects_to_teacher_id == teacher.id)) && "hidden"}"} phx-click="assign_subjects_to_teacher_id" phx-value-teacher_id={teacher.id}>Assign Subjects</.button>
            </div>
            <div class="col-span-2">
          ...
          <%= if @assign_subjects_to_teacher_id == teacher.id do %>
            <div class="relative border p-3 rounded-lg">
              <div phx-click="assign_subjects_to_teacher_id" phx-value-teacher_id="0" class="absolute top-2 right-2 cursor-pointer">
                &#9746;
              </div>
              <div class="p-2 mt-2">
                <h2 class="font-bold mb-2">Choose class and assign subjects to Miss <%= teacher.name %> </h2>
                <div class="flex gap-2">
                  <.form :let={s} for={%{}} as={:class_of_subject} phx-change="class_of_subject">
                    <.input field={s[:class]} type="select" label="Class" options={Enum.flat_map(@classes, fn class -> ["#{class.name}": class.id] end)} />
                  </.form>
                  <%= if @is_class_choosen do %>
                    <div>
                      <.form :let={f} for={%{}} as={:class_of_subject} phx-value-teacher_id={teacher.id} phx-submit="assign_subjects_to_teacher">
          ...
          <%= if @delete_teacher_id == teacher.id do %>
            <div class="relative border p-3 rounded-lg w-1/2">
              <div phx-click="delete_teacher_id" phx-value-teacher_id="0" class="absolute top-2 right-2 cursor-pointer">
                &#9746;
              </div>
              <div class="p-2 mt-2">
                <h2 class="font-bold mb-2">Subjects of Miss <%= teacher.name %> should be transfered to:</h2>
                <.form :let={s} for={%{}} as={:subject_teacher_change} phx-value-prev_teacher_id={teacher.id} phx-submit="subject_teacher_change">
                  <.input field={s[:teacher_id]} type="select" label="Teacher" options={Enum.flat_map(@teachers, fn teacher -> ["#{teacher.name}": teacher.id] end)} />
          ...
          <%= if @edit_teacher_id == teacher.id do %>
            <div class="relative border p-3 w-1/2 rounded-lg">
              <div phx-click="edit_teacher_id" phx-value-teacher_id="0" class="absolute top-2 right-2 cursor-pointer">
                &#9746;
              </div>
              <.form :let={f} for={@teacher_changeset} phx-change="teacher_validation" phx-submit="teacher_updation" phx-value-teacher_id={teacher.id}>
