defmodule TheArkWeb.TeacherShowLive do
  ...
  def handle_event(
        "teacher_updation",
        %{"teacher" => params},
        %{assigns: %{teacher: teacher}} = socket
      ) do
    ...
    <div>
      <div class="flex justify-between items-center mb-10">
        <div class="flex items-center">
          ...
          <span :if={@teacher.is_leaving} class="ml-2 text-xs p-0.5 px-1 border bg-red-200 rounded-lg">
            non-active
          </span>
        </div>
        <div class="flex gap-1 ml-2">
          ...
          <%= if !@teacher.is_leaving do %>
            <.button class="" phx-click={show_modal("teacher_leaving")}>
              Is Leaving?
            </.button>
          <% else %>
            <.button class="" phx-click="re_avtive_teacher">
              Re-active
            </.button>
          <% end %>
        </div>
      ...
      <.modal id="teacher_leaving">
        <.form :let={f} for={@teacher_changeset} phx-submit="teacher_updation_leave">
